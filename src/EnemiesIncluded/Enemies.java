...
public class Enemies extends GameState {
	...
	public void start() {
		...
		for (int d=1; d<4; d++){
			...
			enemyList.add(new Enemies(200+d*100, 480, 0.0f, 0.0f, 50, 50, BoundingBoxNull, BoundingBoxRed, BoundingBoxPurple, false)); //I have to make the parameters linked to the X and Y coordinates??
	...
	public static void intersection() {
			...
			ePolices.add(new Rectangle(dragonas.BoundingBoxNull.getX()+3, dragonas.BoundingBoxNull.getY(), enemyDragonTex.getWidth()-7, enemyDragonTex.getHeight()/3));
			...
			for(Enemies dragonas: enemyList) {
				...
				if(marioPurBox.intersects(dragonas.BoundingBoxPurple)){
					killMario = true;
					System.out.println("KILLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL");
				}
			...
			for(Enemies dragonas: enemyList ){
				...
				Rectangle rec = dragonas.BoundingBoxNull;
